...
-- Восстанавливаем последовательности (если использовались)
-- ALTER SEQUENCE users_id_seq RESTART; -- если используется serial, но у нас UUID, не нужно.

-- Фиксированный хеш для пароля 'test123' (замените на свой!)
-- Пример: $2b$10$N9qo8uLOickgx2ZMRZoMy.MrIy2eZ9tYq5o0G7XQyJ3q7Y8q0q0qO
DO $$
DECLARE
...
BEGIN
  ...
  FOR i IN 1..50 LOOP
    -- Выбор пола (50% мужчины, 50% женщины)
    IF random() < 0.5 THEN
      gender := 'male';
    ...
    show_all := random() > 0.5; -- 50% шанс
    -- Случайные координаты в пределах Москвы (широта 55.5-55.9, долгота 37.3-37.9)
    lat := 55.5 + random() * 0.4;
    lon := 37.3 + random() * 0.6;
  ...
  -- Генерация 10 событий (разные виды спорта, даты в будущем)
  DECLARE
  ...
  BEGIN
    FOR j IN 1..10 LOOP
      -- Случайный создатель из пользователей
  ...
  -- Добавляем участия в событиях (каждый пользователь участвует в 0-3 событиях)
  DECLARE
    event_cursor CURSOR FOR SELECT id FROM events;
    event_rec RECORD;
    user_participations INTEGER;
  BEGIN
    FOR event_rec IN SELECT id FROM events LOOP
      -- Количество участников на событие: случайное от 1 до 20
  ...
  -- Добавляем взаимные мэтчи (10 случайных пар, которые еще не в мэтче)
  DECLARE
    k INTEGER;
    user1 UUID;
    user2 UUID;
  ...
  -- Добавляем несколько договорённостей на события (для пар, у которых есть взаимный мэтч и общее событие)
  DECLARE
    match_rec RECORD;
    common_event_id UUID;
  BEGIN
    FOR match_rec IN SELECT from_user_id, to_user_id FROM matches WHERE status = 'accepted' LOOP
      -- Находим общее событие, где оба участника имеют статус 'looking'
